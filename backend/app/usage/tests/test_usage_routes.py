...
@pytest.mark.anyio
@pytest.mark.parametrize("path", ["/totals", "/by-agent", "/by-model", "/projections"])
async def test_usage_stubs_return_self_healing_available_false(path):
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get(f"/api/v2/usage{path}")
