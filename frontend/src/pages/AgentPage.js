...
export default function AgentPage() {
  ...
  const [capabilities, setCapabilities] = useState({ imageGen: true, styleRef: false, variations: true, promptAssist: false });
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setAgentMessages(prev => [...prev, { id: crypto.randomUUID(), role: "agent", content: `Design placeholder active with ${activeCaps.length} capability settings enabled. Live image generation wiring will be connected later.`, caps: activeCaps }]);
      setIsProcessing(false);
    }, 2000);
  };

  const capButtons = [
    { key: "imageGen", label: "Image Prompting", icon: Image, desc: "Prepare prompts for generated visuals" },
    { key: "styleRef", label: "Style Direction", icon: Paintbrush, desc: "Set style and composition references" },
    { key: "variations", label: "Variation Sets", icon: Code2, desc: "Draft alternate output directions" },
    { key: "promptAssist", label: "Prompt Assistant", icon: Globe, desc: "Refine prompts before generation" },
  ];
        ...
        <div className="flex-1 flex items-center justify-center px-6">
          <div className="w-full max-w-lg">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold mb-1">Design</h1>
              <p className="text-sm" style={{ color: C.muted }}>Image-first design workspace with placeholder generation flow</p>
            </div>
            ...
            {connectedCount === 0 && (
              <Link to="/customize?tab=desktop" className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm mb-4" style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)", color: C.yellow }}>
                <AlertCircle className="w-4 h-4" /> Configure design connectors in Customise <ChevronRight className="w-3.5 h-3.5 ml-auto" />
              </Link>
            )}
            <div className="rounded-2xl" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              <textarea value={agentInput} onChange={e => setAgentInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                placeholder="Describe the image or visual direction you want to create..." rows={2}
              ...
              <div className="flex items-center justify-between px-3 pb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={handleSend} disabled={!agentInput.trim()} className="h-8 px-4 rounded-lg text-sm font-medium" style={{ background: agentInput.trim() ? C.accent : C.surface2, color: agentInput.trim() ? "#fff" : "#555" }}
                    data-testid="agent-send-btn">Generate Draft</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <>
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              ...
              {isProcessing && (
                <div className="flex gap-3"><div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: "linear-gradient(135deg, #ec4899, #8b5cf6)" }}><Sparkles className="w-3.5 h-3.5 text-white" /></div>
                  <div className="flex items-center gap-2 text-sm"><span className="w-2 h-2 rounded-full animate-pulse" style={{ background: "#8b5cf6" }} /><span style={{ color: C.muted }}>Preparing design draft...</span></div>
                </div>
              )}
              <div ref={bottomRef} />
            </div>
          ...
          <div className="px-4 pb-4 pt-2 shrink-0">
            <div className="max-w-2xl mx-auto">
              <div className="rounded-2xl" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
                <textarea value={agentInput} onChange={e => setAgentInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                  placeholder="Refine the design direction..." rows={1} className="w-full focus:outline-none resize-none text-sm" style={{ background: "transparent", border: "none", color: C.text, padding: "12px 16px 8px" }} />
