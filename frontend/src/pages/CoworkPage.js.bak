...
export default function CoworkPage() {
  ...
  const { models, providers, activeModel, connectors, toggleConnector, plugins, togglePlugin } = useGateway();
  ...
  const [coworkSub, setCoworkSub] = useState(null);
  ...
  useEffect(() => {
    ...
    const h = (e) => { if (coworkPlusRef.current && !coworkPlusRef.current.contains(e.target)) { setCoworkPlusOpen(false); setCoworkSub(null); } };
            ...
            <div className="rounded-xl mb-6 overflow-hidden" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              ...
              {showConnectors && (
                <div style={{ borderTop: `1px solid ${C.border}` }}>
                  ...
                  {/* Desktop App Groups (placeholder only) */}
                              ...
                              {group.apps.map(app => {
                                ...
                                return (
                                  <div key={app.id} className="flex items-center gap-2 py-[4px]">
                                    <AppIcon className="w-3 h-3 shrink-0" style={{ color: "#555" }} />
                                    <span className="text-[11px] flex-1 truncate" style={{ color: "#777" }}>{app.label}</span>
                                    <span className="text-[9px] px-1.5 py-0.5 rounded-full" style={{ background: C.surface2, color: C.muted, border: `1px solid ${C.border}` }}>Placeholder</span>
                                  </div>
                                );
                              })}
                            </div>
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5" style={{ borderTop: `1px solid ${C.border}`, background: "rgba(255,255,255,0.01)" }}>
                    ...
                    <Link to="/customize?tab=desktop" className="flex items-center gap-1.5 text-xs" style={{ color: C.muted }}>
                      <Wrench className="w-3 h-3" /> Desktop placeholders
                    </Link>
                  </div>
                </div>
              )}
            </div>
            <div className="rounded-xl mb-6 p-3" style={{ background: C.surface, border: `1px dashed ${C.border}` }}>
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium">Desktop operations placeholder</div>
                  <div className="text-xs" style={{ color: C.muted }}>Future desktop operations for Cowork will appear here after approval-flow wiring.</div>
                </div>
                <Link to="/customize?tab=desktop" className="text-xs" style={{ color: C.accent }}>Open Customise</Link>
              </div>
            </div>
            ...
            <div className="rounded-2xl shadow-xl" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              ...
              <div className="flex items-center gap-2 px-3 pb-3">
                <div ref={coworkPlusRef} className="relative">
                  <button onClick={() => { setCoworkPlusOpen(v => !v); setCoworkSub(null); }}
                  ...
                  {coworkPlusOpen && (
                    <div className="absolute bottom-full left-0 mb-2 z-50 flex items-end gap-1" onMouseLeave={() => setCoworkSub(null)}>
                      <div className="w-64 rounded-xl py-1 shadow-2xl max-h-[360px] overflow-y-auto" style={{ background: "#151515", border: "1px solid #2a2a2a" }}>
                        ...
                        <button type="button" onMouseEnter={() => setCoworkSub("plugins")}
                          className="w-full flex items-center gap-2 px-3 py-[6px] text-[11px] transition-colors hover:bg-white/[0.03]" style={{ color: C.text }}>
                          ...
                          return (
                            <div key={group.id}>
                              <button type="button" onClick={() => setExpandedGroup(isExp ? null : group.id)}
                                ...
                                <span className="text-[9px] px-1 rounded-full" style={{ background: C.surface2, color: C.muted }}>Placeholder</span>
                              ...
                              {isExp && group.apps.map(app => {
                                ...
                                return (
                                  <div key={app.id} className="flex items-center gap-2 pl-7 pr-3 py-[4px] transition-colors hover:bg-white/[0.03]">
                                    <AppIcon className="w-3 h-3 shrink-0" style={{ color: "#555" }} />
                                    <span className="text-[10px] flex-1" style={{ color: "#777" }}>{app.label}</span>
                                    <span className="text-[9px] px-1 rounded" style={{ background: C.surface2, color: C.muted, border: `1px solid ${C.border}` }}>Soon</span>
                                  </div>
                                );
                              })}
                            </div>
                        ...
                        <div className="my-1" style={{ height: 1, background: "#1e1e1e" }} />
                        <button onClick={() => { setCoworkPlusOpen(false); setCoworkSub(null); navigate("/customize?tab=desktop"); }}
                          className="w-full flex items-center gap-2 px-3 py-[6px] text-[11px] transition-colors hover:bg-white/[0.03]" style={{ color: C.muted }}>
                          <Wrench className="w-3 h-3" /> Desktop placeholders
                        </button>
                      </div>
                      {coworkSub === "plugins" && (
                        <div className="w-56 rounded-xl py-1 shadow-2xl max-h-[300px] overflow-y-auto" style={{ background: "#151515", border: "1px solid #2a2a2a" }}>
                          {plugins.map(plugin => (
                            <button key={plugin.id} type="button" onClick={() => togglePlugin(plugin.id)}
                              className="w-full flex items-center justify-between gap-2 px-3 py-[6px] text-[11px] text-left transition-colors hover:bg-white/[0.03]" style={{ color: C.text }}>
                              <span className="truncate">{plugin.name}</span>
                              {plugin.installed && <Check className="w-3 h-3 shrink-0" style={{ color: C.accent }} />}
                            </button>
                          ))}
                        </div>
                      )}
