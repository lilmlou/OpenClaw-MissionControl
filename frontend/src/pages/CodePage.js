...
import { Trash2, Terminal, Monitor, Cloud, ArrowRight, Mic, Copy, Check, ChevronDown, ChevronRight, X, ShieldCheck } from "lucide-react";
...
export default function CodePage() {
  ...
  return (
    <div className="h-full flex flex-col" style={{ color: C.text }}>
      ...
      <div className="px-4 py-2.5" style={{ borderBottom: `1px solid ${C.border}`, background: "#0b0b0b" }}>
        <div className="flex flex-wrap items-center gap-2">
          <Link to="/customize?tab=desktop" className="text-xs px-2.5 py-1.5 rounded-lg" style={{ background: C.surface2, color: C.accent, border: `1px solid ${C.border}` }}>
            Desktop placeholders
          </Link>
          <Link to="/customize?tab=connectors" className="text-xs px-2.5 py-1.5 rounded-lg" style={{ background: C.surface2, color: C.muted, border: `1px solid ${C.border}` }}>
            Connectors
          </Link>
          <span className="text-[11px] px-2.5 py-1 rounded-full flex items-center gap-1" style={{ background: "rgba(251,191,36,0.08)", color: C.yellow, border: "1px solid rgba(251,191,36,0.2)" }}>
            <ShieldCheck className="w-3 h-3" /> Desktop operations are placeholders in this step
          </span>
        </div>
      </div>
      ...
      <div className="p-3 md:p-4" style={{ borderTop: `1px solid ${C.border}`, background: "#0a0a0a" }}>
        <div className="max-w-3xl mx-auto space-y-2 md:space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <span className="text-[11px] px-2 md:px-3 py-1.5 rounded-lg" style={{ background: C.surface2, color: C.muted, border: `1px dashed ${C.border}` }}>
                Desktop operation controls: placeholder only
              </span>
            </div>
            <div className="flex items-center gap-2">
