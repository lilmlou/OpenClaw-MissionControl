    ...
    return (
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        ...
        <div
          className="absolute inset-0"
          style={{
            background: `radial-gradient(circle at 50% 22%, ${background.accentGlow} 0%, rgba(255, 48, 78, 0) 34%)`,
            mixBlendMode: "screen",
          }}
        />
      </div>
...
export function Toggle({ on, onToggle, accentColor = ORIGINAL_FRONTEND_THEME.accent }) {
  return (
    <button type="button" onClick={onToggle} className="w-8 h-[18px] rounded-full relative transition-colors shrink-0" style={{ background: on ? accentColor : "#333" }}>
      <div className="absolute top-[2px] w-[14px] h-[14px] rounded-full transition-all" style={{ background: "#fff", left: on ? 14 : 2 }} />
    </button>
  );
}

export function Markdown({ content, theme = C }) {
  return (
    <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
      ...
      code: ({ children, className }) => className
        ? <code className="block text-[11px] rounded p-2 my-1.5 overflow-auto font-mono whitespace-pre-wrap" style={{ background: theme.surface2, border: `1px solid ${theme.border}` }}>{children}</code>
        : <code className="text-[11px] rounded px-1 py-0.5 font-mono" style={{ background: theme.surface2, border: `1px solid ${theme.border}` }}>{children}</code>,
      pre: ({ children }) => <>{children}</>,
      a: ({ href, children }) => <a href={href} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:opacity-80" style={{ color: theme.accent }}>{children}</a>,
